...
The TypeScript implementation passed all 59 tests on Node 24.15. The package test installs the tarball into an isolated prefix and runs `repo-queue --version` outside the checkout. No Python runtime is required, including for the legacy SQLite migration fixture. Real-process regressions hold a shared status read during startup and verify that one dispatcher acquires its lease and delivers the turn. Concurrent starts run alongside eight status clients. A stop generation fences pending launches across lease release; tests require both competing processes to exit and invoke the actual CLI stop while a reader blocks acquisition, proving that the pending launch cannot start afterward. An immediate CLI stop/start then delivers newly queued work. Delayed cleanup after a delivery timeout retains the four-process limit and per-owner serialization; stopping the dispatcher leaves those agents alive to finish. A durable delivery ledger preserves the four slots and owning-session serialization across stop/start, even if queue tokens change. The restart exercise stays at four live commands, then admits waiting work after the original processes exit. Orphan reconciliation requires the current entry token and explicit quiescence, preserves completed entries, and refuses live tracked processes. A ledger write failure waits for natural child closure without terminating the agent. Generated wake commands claim, verify and complete a turn with a leading-dash token; Claude discovery and relative configuration lookup use the owner directory independently of the dispatcher launch directory. Claude resume fixtures accept ordinary result objects and verbose event arrays while rejecting incorrect session identities or ambiguous results. A separate synthetic version-0 migration check preserved a mixed queue containing every state, existing tokens and order; interrupted sending became uncertain without releasing ownership.
...
A two-configuration exercise registers a Claude owner under directory B and starts the actual dispatcher under directory A. The simulated Claude executable observes B for discovery and resume; delivery is sent while the reservation and token remain unchanged. Automated coverage also checks Codex and live Claude routing, defaults, conflicting registrations, failure retention and claim verification. A separate check uses the installed Python CLI to claim and complete a new entry carrying owner metadata; the TypeScript reader preserves that metadata and the original ID/token.

A private-socket regression covers a literal percent escape in a Claude socket path. The native address is encoded before sending; decoding it selects the exact validated socket, and the fixture establishes a real local connection to that socket.
...
An idle dispatcher sample on macOS with Node 24.15 reported 53,888 KiB resident memory and 0.0% CPU through `ps`. This is one observation, not a benchmark or resource guarantee; agent processes have their own resource use.
