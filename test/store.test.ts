...
describe("Store", () => {
  ...
  test("captures each owner's resolved configuration root and refuses to retarget it", () => {
    ...
    try {
      ...
      const defaultCodex = add(item, "https://github.com/acme/codex-default/pull/3");
      ...
      assert.deepEqual(columns, ["entry_id", "config_root"]);
  ...
  test("opens a version-0 database written by the Python Store without migration", () => {
    ...
    assert.equal(version.prepare("PRAGMA user_version").get()?.user_version, 1);
    version.close();
    migrated.close();
  });
